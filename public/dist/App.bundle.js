...
function loadPlaces(map) {
	...
	_axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
		...
		var bounds = new google.maps.LatLngBounds(); //get the bounds of the map so we can center later
		var infoWindow = new google.maps.InfoWindow();

		var markers = places.map(function (place) {
			var _place$location$coord = _slicedToArray(place.location.coordinates, 2),
			    placeLng = _place$location$coord[0],
			    placeLat = _place$location$coord[1]; // create new variables
		...
		// when someone click on marker, show location detail with InfoWindow stuff
		...
		// then set map center to center of locations
	...
	searchInput.on('input', function () {
		...
		_axios2.default.get('/api/search?q=' + this.value).then(function (res) {
			if (res.data.length) {
				...
				searchResults.innerHTML = _dompurify2.default.santize('<div class=\'search__results\'>No search results found</div>');
			}
		}).catch(function (err) {
			console.error(err);
		});
...
var heartForms = (0, _bling.$$)('form.heart'); //any forms that have class of heart
